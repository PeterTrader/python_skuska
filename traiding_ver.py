...
# API kľúče
api_key = "G0i9qBVHGr1UNs2FmQkYkPCd4L9i6nRac55WDATUaXdASP8po9VsmQdlOorxPZPD"
api_secret = "NKdolnpl1nK6lCifUYf9egq9g44OMgggYjBS4LY7IiU4cdP3RluRyjubpDhhMqel"
...
MAX_OPEN_ORDERS = 40  # Maximálny počet otvorených objednávok
PAUSE_ON_LOSS_RATIO = 2.0  # Ak stratových je 2x viac ako ziskových, pauzni
PAUSE_TIME = 60  # sekúnd pauzy pri extrémnej situácii
...
def get_dynamic_thresholds(symbol):
    ...
    if vol > 100:
        profit_levels = [0.9, 1.1, 1.3, 1.5]
        loss_limit = 0.5
    elif vol > 50:
        profit_levels = [0.25, 0.35, 0.5, 0.7]
        loss_limit = 0.3
    else:
        profit_levels = [0.15, 0.2, 0.3, 0.4]
        loss_limit = 0.15
    ...
    while not closed:
        try:
            ...
            for i, level in enumerate(profit_levels):
                key = f"{level:.2%}"
                if key not in closed_partials and profit_percent >= level:
            ...
            if profit < 0:
                ...
                logging.info(f"Kontrola straty: {loss_percent:.4f}% (limit: {loss_limit:.2f}%) pre order {qty}")
                if loss_percent >= loss_limit:
                    close_side = "SELL" if side == "BUY" else "BUY"
...
def monitor_orders():
    ...
    with order_lock:
        ...
        print(f"\nCelkový profit od spustenia: {total_profit:.8f}")
...
last_trade_time = 0
trade_interval = 15  # sekúnd medzi obchodmi (rýchlejšie otváranie)
last_side = "SELL"  # Začíname BUY, potom sa bude striedať
            ...
            if now - last_trade_time > trade_interval:
                ...
                if last_side == "SELL":
                    ...
                    if order:
                        ...
                        # Spustenie monitorovania cez thread pool
